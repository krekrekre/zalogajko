        ...
        <div className="flex w-full flex-col gap-6 sm:gap-8 lg:flex-row lg:items-start lg:justify-start">
          ...
          <div className="min-w-0 flex-1 lg:max-w-xl">
            <h2 className="font-display flex items-center gap-2 text-lg font-semibold text-[var(--color-primary)] sm:text-2xl">
              <span className="flex h-8 w-8 items-center justify-center rounded-none bg-[var(--ar-cream)]">
                <ChefHat
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-5">
              {POPULAR_SEARCHES.slice(0, 8).map((search) => (
                <Link
                  key={search.href}
                  href={search.href}
                  className="rounded-none bg-[var(--ar-primary)] px-3 py-2 text-center text-xs font-medium text-white transition-colors hover:opacity-90 sm:text-sm"
                >
                  {search.label}
                </Link>
              ))}
