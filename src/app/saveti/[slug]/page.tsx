...
export default async function SavetiArticlePage({ params }: Props) {
  ...
  const breadcrumbItems = [
    { name: "Saveti", path: "/saveti" },
    { name: article.title, path: `/saveti/${article.slug}` },
  ];
        ...
        <div className="mt-8 w-full bg-[#f1f1e6] py-10 sm:py-12">
          <div className="mx-auto max-w-[1220px] px-4 sm:px-6 lg:px-8">
            ...
            <div className="mt-6 grid gap-4 sm:mt-8 sm:grid-cols-2 sm:gap-6 lg:grid-cols-3 xl:grid-cols-4">
              {related.map((a) => (
                <ArticleCard key={a.slug} article={a} basePath="/saveti" />
              ))}
            </div>
          </div>
        </div>
